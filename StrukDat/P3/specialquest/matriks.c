...
Matriks maxPooling(Matriks M, int size){
    ...
    //algoritma
    initMatriks(&M2);
    M2.nbaris = (M.nbaris / size) + M.nbaris % size;
    M2.nkolom = (M.nkolom / size) + M.nkolom % size;
    for(i = 1; i <= M2.nbaris; i++){
        for(j = 1; j <= M2.nkolom; j++){
            temp = -999;
            for(k = 1; k + i<= M.nbaris&& k <= size; k++){ 
                for(l = 1; l + j <= M.nkolom && l <= size; l++){
                    if(M.cell[k][l] > temp) temp = M.cell[k][l];
                }
            }
            M2.cell[i][j] = temp;
        }
...
Matriks conv(Matriks M, Matriks K){
    ...
    for (i = 1; i <= M2.nbaris; i++){
        for (j = 1; j <= M2.nkolom; j++){
            sum = 0;
            for (k = 1; k <= K.nbaris; k++){
                for (l = 1; l <= K.nkolom; l++){
...
/* function flr_sqrt(x: integer) -> integer
    {mengembalikan akar bulat terbesar dari x}*/
int flr_sqrt(int x) {
    //kamus
    int i = 1;
    //algoritma
...
void createMatriksZero(Matriks *M, int n, int m) {
    //kamus
    int i, j;
    //algoritma
    (*M).nbaris = n;
